...
public class MyOrderPage extends Setup {
    ...
    public CheckoutPage clickReOrderBtn() {
        clickReorderAction();
        if (!helper.waitForURLContains(checkoutDataTest.URL)) {
            clickReorderAction();
        }
        System.out.println(checkoutDataTest.URL);
        return new CheckoutPage(driver);
    }

    private void clickReorderAction() {
        ...
        if (helper.checkDisplayElement(confirmModal)) {
            ...
            if (driver.findElement(confirmDialogContent).getAttribute("innerHTML").equals(jsonReader.localeReader(getCurrentLanguage(), myProfileDataTest.PAGE, keyList))) {
                try {
                    helper.clickBtn(driver.findElement(confirmButtonWithFooter));
